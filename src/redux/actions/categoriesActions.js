...
export function deleteCategory(categoryName) {
	return (dispatch) => {
		let apiUrl = 'http://localhost:3000/api/v1/videoCategories/del/' + categoryName;
		axios
			.delete(apiUrl)
			.then(() => {
				dispatch(setCategoriesDirty());
...
export function moveCategoryUp(categoryName) {
	return (dispatch) => {
		let apiUrl = 'http://localhost:3000/api/v1/videoCategories/up/' + categoryName;
		axios
			.put(apiUrl)
			.then(() => {
				dispatch(setCategoriesDirty());
...
export function moveCategoryDown(categoryName) {
	return (dispatch) => {
		let apiUrl = 'http://localhost:3000/api/v1/videoCategories/down/' + categoryName;
		axios
			.put(apiUrl)
			.then(() => {
				dispatch(setCategoriesDirty());
